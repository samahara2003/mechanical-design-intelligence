...
The displacement quantity was declared before refinement: area-average global UX over the named load-pad face. It is obtained by exactly integrating the quadratic interpolation on each straight six-node surface triangle. The three-level sequence follows a constant `1.5` characteristic-size ratio: coarse `0.009 m`, baseline `0.006 m`, and fine `0.004 m`. The meshes contain respectively 8,108/4,109, 17,314/9,577, and 47,836/28,848 nodes/C3D10 elements. The existing corner-tetrahedron quality diagnostics remain evidence against invalid or obviously distorted corner tetrahedra, not a complete characterization of curved quadratic-element Jacobians. Gmsh reported no warning at any level.

| Quantity | Coarse, 9 mm | Baseline, 6 mm | Fine, 4 mm |
| --- | ---: | ---: | ---: |
| Nodes / C3D10 elements | `8,108 / 4,109` | `17,314 / 9,577` | `47,836 / 28,848` |
| Load-pad area-average UX | `0.573020 mm` | `0.580187 mm` | `0.582652 mm` |
| Global maximum displacement magnitude | `0.885308 mm` | `0.895230 mm` | `0.898831 mm` |
| Global raw integration-point von Mises | `143.417 MPa` | `145.277 MPa` | `164.412 MPa` |
| Force-equilibrium residual | `3.03e-8 N` | `4.04e-8 N` | `3.09e-8 N` |

At all three levels the global maximum displacement occurs at the top centre of the upright, approximately `(0.140, 0.050, 0.120) m`, and combines positive X motion with downward bending under the eccentric `+X` load. Constrained-hole nodes remain at zero displacement. Applied/reaction force pairs are `(2500,0,0) N` and approximately `(-2500,0,0) N`; moments assembled about the origin also balance to within the recorded numerical residuals. CalculiX 2.23 completed all three jobs without warnings.

The coarse, baseline, and fine raw peaks are `143.417307`, `145.277077`, and `164.411765 MPa`. All fall in the predeclared mounting-hole vicinity, which is mechanically plausible under the idealized fully fixed bores. This is reported directly as local mesh/idealization sensitivity; it is not hidden, called design-critical, converted to FoS, or used for pass/fail.

The same displacement evidence exercises the reusable `QuantityOfInterest` and `QuantityEvaluation` contracts. `load_pad_average_ux` targets the named geometry face, not resolved node IDs. The fresh coarse, baseline, and fine values are `0.0005730201096575444`, `0.000580187226580943`, and `0.0005826523352911262 m`. Every level records its actual C3D10 mesh summary, content-addressed mesh identity, the resolved `gmsh_physical_surface:load_pad` identity, and the same non-size analysis-comparison fingerprint.

Policy `controlled_bracket_load_pad_average_ux/1` gives coarse-to-baseline absolute/relative changes of `7.167116923398678e-6 m` / `1.2507618498209395%` (`outside_tolerance`) and baseline-to-fine changes of `2.4651087101831056e-6 m` / `0.4248815894672568%` (`within_tolerance`). Because both successive change measures strictly decrease, the three-level study reports `stabilizing` under scope `three_level_mesh_trend_only`. Its explicit minimum reference magnitude remains `1e-12 m`; an unavailable adjacent relative change makes the trend `indeterminate`. The 1% threshold is controlled-case verification policy, and `stabilizing` does not demonstrate mathematical convergence, an exact solution, structural correctness, or physical validation.

The displacement sequence is eligible for the narrower constant-ratio three-grid estimate. With grid 3/coarse, grid 2/baseline, grid 1/fine and `r32 = r21 = 1.5`, V1 uses `p = ln(|(phi2-phi3)/(phi1-phi2)|) / ln(r)`. The observed order is `2.6322056859000242`. The fine/medium Richardson formula `phi_ext = phi1 + (phi1-phi2)/(r^p-1)` gives `0.000583944710947966 m` (`0.583944710947966 mm`), which is `1.2923756568397962e-6 m` above the fine result; its absolute difference relative to the extrapolated magnitude is `0.22131815437488518%`.

Policy `controlled_bracket_three_grid_discretization_estimate/1` uses the standard three-grid safety factor `1.25`, an explicit `1e-12 m` minimum for successive differences and relative denominators, and a `1e-12` relative tolerance for equality of refinement ratios. The approximate fine/medium relative error is `0.42308398351332405%`; `GCI_21 = 1.25 |(phi1-phi2)/phi1|/(r^p-1)` is `0.0027726132261060472` as a dimensionless fraction, or `0.2772613226106047%`. Applying the same convention to coarse/medium gives `GCI_32 = 1.25 |(phi2-phi3)/phi2|/(r^p-1) = 0.008095413417591617`, or `0.8095413417591617%`. These are estimated discretization uncertainties for this one QoI under the stated method assumptions. The extrapolated result is not called the true physical solution.

The separate policy `controlled_bracket_asymptotic_consistency/1` checks `GCI_32 / (r^p GCI_21)` against target `1.0`. Its controlled-case absolute-deviation tolerance is `0.01`, and its explicit minimum denominator is `1e-15`; neither value is a universal FEA rule. The resulting ratio is `1.0042488158946725`, with absolute deviation `0.0042488158946725`, so the status is `consistent`. This status describes only the configured adjacent-GCI numerical relation. It does not independently prove an asymptotic range, exactness, universal convergence, structural correctness, design acceptance, or physical validation; the broader asymptotic-range assessment therefore remains `not_established_v1`.

The raw integration-point von Mises diagnostic changes from coarse to baseline by `1.8597704905398786 MPa` (`1.296754576008271%`) and from baseline to fine by `19.13468707153815 MPa` (`13.17116740637191%`). Its observed change trend is therefore `not_stabilizing`. The structured record retains every peak identity and location but remains `diagnostic_only`: it has no threshold, design-critical designation, FoS, failure status, singularity claim, or stronger convergence conclusion.

Applying the same formal-estimate eligibility gate to that raw-stress sequence returns `ineligible` with reason `observed_order_not_positive`: the second successive change is larger, so the V1 order formula would not yield a positive order. Its observed-order, Richardson, approximate-error, and GCI fields are therefore null. No discretization estimate is manufactured for the diagnostic peak.
...
Applying `controlled_bracket_critical_stress_establishment/1` to those existing records returns `not_established` and emits no numeric design-stress reference. The deterministic blockers are `global_raw_peak_is_diagnostic_only`, `global_raw_peak_mesh_sensitive`, `feature_specific_extraction_not_defined`, `regional_local_maximum_mesh_sensitive`, `regional_maximum_location_not_stable`, and `model_assumption_limitations_unresolved`. These follow from the raw peak's non-stabilizing trend, mesh-sensitive upright-band maximum, material movement of the declared regional maximum, absence of a justified feature-specific extraction method, and the unresolved fully fixed mounting-bore idealization. The stable base-transfer mean does not override the local-maximum evidence because averaging has not been justified as preserving or resolving a physically meaningful peak.

All four bands are retained as `investigation_only` candidate regions because each has a mesh-sensitive local maximum; none is labeled design-critical or ranked by stress magnitude. Required next evidence is structured as feature-specific local refinement, feature-specific extraction convergence, localized peak tracking, explicit extraction methodology with local-peak treatment, and characterization of constraint-sensitive stress. A synthetic fully qualified case tests the `established` contract path, but the controlled-bracket numerical evidence is not altered or promoted. This assessment is not a yield, FoS, safe/unsafe, failure, singularity, or physical-validation conclusion.

The first root-feature experiment applies a deterministic Gmsh Box background field over `x=[0.108,0.140] m`, `y=[0,0.100] m`, `z=[0.006,0.036] m`, with a `0.006 m` transition thickness. The `0.006 m` far-field target remains fixed while the local target decreases by ratio `1.5`: `0.006`, `0.004`, and `0.0026666667 m`. The evidence region `root_fillet_neighborhood/1` is the closed box `x=[0.110,0.135] m`, `y=[0,0.100] m`, `z=[0.008,0.032] m`; it encloses the full 15 mm fillet projection plus adjacent base/upright material and remains independent of generated mesh identities. The first level is the reference for this background-field family, not the prior uniform 6 mm mesh: disabling point, curvature, and boundary-extension size sources for deterministic field control changes the generated topology even when `VIn = VOut = 0.006 m`.

| Local target | Nodes / C3D10 elements | Samples | Minimum | Arithmetic mean | Maximum |
| --- | ---: | ---: | ---: | ---: | ---: |
| `6.000 mm` | `16,592 / 9,207` | `4,037` | `1.260405 MPa` | `19.045708 MPa` | `95.709446 MPa` |
| `4.000 mm` | `22,361 / 13,005` | `12,396` | `1.688343 MPa` | `18.680512 MPa` | `112.690635 MPa` |
| `2.667 mm` | `41,406 / 26,088` | `40,377` | `1.374284 MPa` | `18.700601 MPa` | `135.764729 MPa` |

The maximum locations are `(0.112073887, 0.028175791, 0.011167470) m`, `(0.112180228, 0.044796551, 0.011439547) m`, and `(0.112555933, 0.039983170, 0.011645439) m`. Successive movements are `0.016623327 m` and `0.004832409 m`. Policy `controlled_bracket_root_feature_localization/1` permits at most `0.006 m` adjacent movement, so the complete sequence is `not_localized_within_policy`; the final pair alone is closer but does not erase the first movement.

The mean changes by `-0.365196 MPa` (`1.917473%`) and `+0.020090 MPa` (`0.107544%`), yielding `comparatively_stable` under the explicit `5%` controlled mean policy. The maximum increases by `+16.981189 MPa` (`17.742438%`) and `+23.074094 MPa` (`20.475609%`), yielding `mesh_sensitive` under the explicit `10%` controlled maximum policy. No high sample is clipped. Global raw maxima remain separate diagnostics at `161.410526`, `160.983533`, and `161.018453 MPa`.
