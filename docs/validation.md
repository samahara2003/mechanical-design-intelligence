...
Policy `controlled_bracket_three_grid_discretization_estimate/1` uses the standard three-grid safety factor `1.25`, an explicit `1e-12 m` minimum for successive differences and relative denominators, and a `1e-12` relative tolerance for equality of refinement ratios. The approximate fine/medium relative error is `0.42308398351332405%`; `GCI_fine = 1.25 |(phi1-phi2)/phi1|/(r^p-1)` is `0.0027726132261060472` as a dimensionless fraction, or `0.2772613226106047%`. This is an estimated discretization uncertainty for this one QoI under the stated method assumptions. V1 does not perform an independent asymptotic-consistency check, so asymptotic-range confirmation remains explicitly not established. The extrapolated result is not called the true physical solution.
