...
## Realistic mounting-bracket integration case

The first non-prismatic Engineering Core case is a single-solid steel mounting bracket. Its `140 mm x 100 mm x 12 mm` base supports a `12 mm` thick, `120 mm` high upright through a `15 mm` root radius. Two `12 mm` diameter through-holes represent mounting locations, and a protruding `40 mm x 40 mm` end pad provides a distinct load-transfer face. The geometry is deterministic and dimensioned in SI, but it is an integration fixture rather than a released product design.

Engineering Worker V0 recognizes only this controlled bracket contract and the existing geometric selection checks. Uploading another STEP file does not imply it can be analyzed. The 30-minute PostgreSQL claim lease is an operational V0 assumption sized for the demonstrated local baseline solve; there is no heartbeat or general long-running-analysis policy yet. Expiry permits another attempt, while a unique claim token prevents the stale attempt from publishing after reclaim.

Both cylindrical mounting-hole faces are fixed in UX, UY, and UZ. This is a deliberately simplified mounting idealization: it does not model bolts, washer contact, preload, friction, clearance, or supporting-structure compliance, and it can elevate local stresses beside the constraints. A `2500 N` global `+X` resultant is represented as uniform vector traction over the pad end face. It is not a detailed model of a pin, fastener, or attached component.

The named `mounting_holes` and `load_pad` surfaces are reconstructed after STEP import using dimension-based bounding boxes and exact surface-count checks. This is auditable for the fixed fixture, but it is not persistent CAD topological naming and may require revised selection rules after a geometry edit. The raw global integration-point stress maximum is retained as a mesh-sensitive diagnostic, not as a failure stress, critical-region policy, or FoS input. Two mesh levels provide limited sensitivity evidence only; the case has no whole-part analytical solution or experimental validation.
