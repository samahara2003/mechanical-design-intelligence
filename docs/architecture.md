...
`DiscretizationErrorEstimate` V1 is a narrow immutable layer over an eligible three-level study. It supports only three positive characteristic sizes ordered grid 3/coarse, grid 2/medium, grid 1/fine with a constant refinement ratio, compatible engineering intent, finite values, monotonic successive differences above explicit policy minima, and a positive finite observed order. Ineligible evidence retains a structured reason and null formal numerical fields. For eligible evidence it records observed order, Richardson-extrapolated value, fine-to-extrapolated difference, approximate relative error, and fine-grid GCI as dimensionless fractions. Policy `controlled_bracket_three_grid_discretization_estimate/1` exposes the three-grid GCI safety factor `1.25`, `1e-12` quantity-unit difference/reference minima, and `1e-12` refinement-ratio relative tolerance. The estimator is solver-neutral and does not assert an asymptotic range, true physical solution, model-form uncertainty, probability, structural status, or validation.

`AsymptoticConsistencyEvidence` V1 is derived from an existing discretization estimate and a separate versioned policy. The estimate exposes `GCI_32` for coarse-to-medium and the unchanged `GCI_21` for medium-to-fine using the same observed order, safety factor, denominator, and pair-relative error convention. Policy `controlled_bracket_asymptotic_consistency/1` evaluates `GCI_32 / (r^p GCI_21)` against target `1.0` with an explicit absolute deviation tolerance of `0.01` and minimum valid denominator `1e-15`; all quantities are dimensionless fractions. `consistent` means only that this adjacent-GCI relation falls within that controlled-case tolerance. It does not establish an exact solution, asymptotic range by itself, universal mesh convergence, physical validation, structural correctness, or design status. Ineligible estimates and unavailable or numerically invalid inputs produce `not_applicable` with null numerical consistency fields.

The companion three-level raw-stress record retains each global raw integration-point peak, mesh context, and both adjacent changes. It applies the same conservative observed-change classification and remains `diagnostic_only`. Its increasing successive changes make the formal discretization estimate ineligible, so adjacent GCI and asymptotic-consistency evidence are `not_applicable`; no stress acceptance policy is introduced.

`RegionalStressEvidence` V1 adds descriptive stress evidence for an immutable physical-coordinate region without placing mesh node, element, or integration-point identities in engineering intent. The controlled case declares `lower_upright_web_stress/1` as the closed global box `x=[0.128,0.140] m`, `y=[0,0.100] m`, `z=[0.030,0.060] m`. This is the lower upright load path above the root fillet and below the load pad. It is separated from the immediate fixed mounting-hole surfaces by a minimum geometric distance of approximately `0.088843 m`, while still preserving the fully fixed-bore assumption as context. Every located raw integration-point tensor inside the box contributes its deterministically derived von Mises value; no high sample is clipped or discarded. The immutable evidence records count, minimum, arithmetic mean, maximum, raw stress representation, mesh identity/summary, units, region provenance, and constraint relationship.

`RegionalStressMeshStudy` V1 requires the same region across exactly three coarse-to-fine meshes and reports signed and relative adjacent changes for the arithmetic mean and maximum. It applies no acceptance threshold or formal convergence machinery. The region is controlled verification evidence only: separation from the constraint does not establish stress accuracy, physical adequacy, or suitability for later design decisions. The global raw peak remains separate `diagnostic_only` evidence and is neither hidden nor replaced.

The controlled `StressSpatialDiagnostic` adds physical localization without turning numerical identities into engineering intent. Regional and band maxima retain raw-integration-point XYZ coordinates, but element and integration-point IDs are used only for deterministic internal tie-breaking and are not serialized. Four non-overlapping half-open global-X bands cover the bracket: support-side base `[0,0.070) m`, base transfer `[0.070,0.113) m`, root-transition projection `[0.113,0.128) m`, and upright/load path `[0.128,0.155] m`. Global X is explicitly a coordinate proxy relative to bracket geometry, not an exact support distance.

Policy `controlled_bracket_stress_spatial_diagnostic/1` defines `0.015 m` as its controlled near-feature distance, `0.010 m` as its localized-maximum distance, `0.020 m` as material movement, `5%` as comparative mean stability, and `10%` as maximum mesh sensitivity. The feature record distinguishes an exact Euclidean distance to the finite cylindrical fixed surfaces from coordinate-only offsets above the fillet top and below the load-pad bottom. Spatial labels report only configured geometric association and observed refinement behavior. The mechanism relating any maximum to the support, fillet, or another feature remains indeterminate.

`CriticalStressAssessment` V1 is the next immutable, solver-neutral boundary: existing raw, regional, refinement, spatial, and modeling-assumption evidence is assessed under the controlled policy `controlled_bracket_critical_stress_establishment/1`; no solver output is recomputed. The policy separately treats the global raw peak as diagnostic, requires a versioned physical region, requires a feature-specific extraction method, requires comparatively stable local-maximum behavior and spatial localization, requires explicit treatment of potentially meaningful local peaks before accepting an aggregate, and requires relevant modeling limitations to be adequately characterized. A global maximum, regional maximum, stable mean, or geometric separation from the support cannot satisfy those criteria by itself.

Status is only `established` or `not_established`. An established record may carry a region-and-method stress reference; a not-established record carries no numeric design stress and instead records stable blocker codes, investigation-only candidate regions, unresolved modeling assumptions, and deterministic next-evidence requirements. This is the boundary `stress evidence -> spatial diagnostics -> CriticalStressAssessment -> future material/FoS assessment`. It adds no yield check, FoS, structural pass/fail, singularity detector, stress-linearization method, physical validation, worker/persistence behavior, or AI interpretation.

`FeatureStressRefinementStudy` V1 adds a separate targeted-local-refinement experiment without replacing the established global `9/6/4 mm` study. `LocalMeshRefinementDefinition` identifies a versioned physical feature and a global-coordinate sizing box, fixed far-field target, varying local target, transition thickness, and concrete Gmsh Box-field method; it contains no generated node, element, or integration-point IDs. The controlled root experiment fixes the far field at `0.006 m`, varies only the root target through `0.006`, `0.004`, and `0.0026666667 m`, and evaluates one immutable physical root-fillet neighborhood on every mesh. Its 6 mm local-field reference is internal to this study: deterministic background-field size-source settings make it a different mesh from the pre-existing uniform 6 mm global mesh.

The feature study retains regional minimum/mean/maximum raw-IP-derived von Mises stress, maximum XYZ location, sample and mesh counts, the separate global raw diagnostic, adjacent statistic changes, and physical maximum-location movement. Its named localization and behavior policies produce descriptive labels only. The existing uniform-global-grid Richardson/GCI contract is explicitly ineligible because this experiment holds the far-field characteristic size fixed while changing a nonuniform local background field; no formal estimate is manufactured. A separate impact record exposes additional evidence to `CriticalStressAssessment` while preserving unresolved blockers and forbidding automatic design-stress promotion.

`StressSpatialProfile` V1 further characterizes that feature without interpolating stress. The controlled path originates at the exact CAD line where the 15 mm quarter-circle fillet is tangent to the base, follows global `-X` inward through the base for `0.012 m`, retains the full bracket width, and accepts located raw integration points within `+/-0.002 m` of the base-top Z coordinate. Four fixed 3 mm half-open distance bins (the last closed at its upper extent) retain every qualifying raw tensor's derived von Mises value, count, minimum, arithmetic mean, maximum, and maximum XYZ location. Empty bins remain explicit null evidence. Adjacent-bin mean differences divided by bin-center spacing are descriptive finite differences in `Pa/m`, not interpolated stresses or continuum derivatives.

`StressSpatialProfileStudy` requires the identical path on the three local meshes. Its controlled policy compares normalized mean-profile shape, defines a high-stress zone as bins whose maximum is at least 80% of that profile's maximum, and reports physical zone width. These labels are profile diagnostics only, not formal stress convergence, stress linearization, hot-spot stress, singularity detection, or design-stress selection. Peak-to-profile association records whether the pre-existing feature maximum lies in the same physical bin on every mesh. An additive assessment-impact record can report improved localization while retaining every critical-stress blocker and a null established-stress reference.
