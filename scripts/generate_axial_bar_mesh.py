"""Generate the axial-bar STEP fixture and C3D10 mesh through Gmsh/OpenCASCADE."""
...
def write_fixture_script(path: Path, step_path: Path) -> None:
    ...
    path.write_text(
        f'''// Deterministic axial-bar fixture. STEP export uses millimetres. Generated; do not edit.
SetFactory("OpenCASCADE");
Box(1) = {{0, 0, 0, {LENGTH_M * scale}, {WIDTH_M * scale}, {HEIGHT_M * scale}}};
...
def write_mesh_script(path: Path, step_path: Path, msh_path: Path, inp_path: Path) -> None:
    path.write_text(
        f'''// Import axial-bar STEP and create benchmark-specific FEA groups. Generated; do not edit.
SetFactory("OpenCASCADE");
...
axialLoad[] = Surface In BoundingBox {{{LENGTH_M} - eps, -eps, -eps, {LENGTH_M} + eps, {WIDTH_M} + eps, {HEIGHT_M} + eps}};

If (#volumes[] != 1)
  Error("Expected exactly one imported axial-bar solid");
EndIf
If (#fixed[] != 1)
...
Printf("MDI_GEOMETRY volumes=%g surfaces=%g fixed=%g load=%g", #volumes[], #surfaces[], #fixed[], #axialLoad[]);
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="artifact directory (default: artifacts/axial_bar)",
    )
...
def main() -> int:
    ...
    fixture_script = output_dir / "generate_axial_bar_fixture.geo"
    mesh_script = output_dir / "mesh_axial_bar_step.geo"
    step_path = output_dir / "axial_bar.step"
    msh_path = output_dir / "axial_bar.msh"
    inp_path = output_dir / "axial_bar_mesh.inp"
    summary_path = output_dir / "axial_bar_mesh_summary.json"
